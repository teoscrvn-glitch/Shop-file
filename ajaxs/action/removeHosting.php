...
if(isset($_POST['action']) && $_POST['action']=='delete'){
    if (isset($_POST['id'])) {
        $id = xss($_POST['id']);
        $row = $LOCNGUYEN_SIEUTHICODE->get_row("SELECT * FROM `tbl_hosting` WHERE `id` = '$id' ");
        if (!$row) {
            $data = json_encode([
                'status'    => 'error',
                'msg'       => 'Gói không tồn tại trong hệ thống'
            ]);
            die($data);
        }
        $isRemove = $LOCNGUYEN_SIEUTHICODE->remove("tbl_hosting", " `id` = '$id' ");
        if ($isRemove) {
            $data = json_encode([
                'status'    => 'success',
